  ...
  long long int counter=0;
	...
	{	
		...
		{	
			...
			{
				c[i]=a[aptr++];
				counter+=(long long int)((long long int)c[i]*(long long int)(right+1-bptr));
			}
			else
			{
			//	counter+=((long long int)((center-aptr)+1)*a[bptr]);
			/*	int u;
				for(u=aptr;u<=center;u++)
				{
					counter+=a[u];
					printf("%d ",a[u]);
				...
				*/
			//	printf("%d * %d ",a[bptr],center-aptr+1);
				c[i]=a[bptr++];
			}
		}
		else
